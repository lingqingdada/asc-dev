...
- 算子实现：  
  ...
  - Kernel实现
    ...
    ReduceSumCustom算子的实现流程分为3个基本任务：CopyIn，Compute，CopyOut。CopyIn任务负责将Global Memory上的输入Tensor srcGm存储在srcLocal中。Compute任务负责将输入srcLocal的前288个数据求和，并将结果存储在dstLocal中。CopyOut任务负责将输出数据从dstLocal搬运至Global Memory上的输出Tensor dstGm。
