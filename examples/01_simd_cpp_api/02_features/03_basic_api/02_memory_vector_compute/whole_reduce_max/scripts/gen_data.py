...
def gen_golden_data_simple():
    ...
    for i in range(repeat):
        #获取最大值
        golden[0 + 2 * i] = np.max(input_x[i * one_repeat_items:(i + 1) * one_repeat_items])
        #获取最大值的下标，并且强制转换为half类型
        max_index = np.argmax(input_x[i * one_repeat_items:(i + 1) * one_repeat_items])
        #max_index的数据类型默认是int64_t，view方法只支持位宽不变
        max_index = np.uint16(max_index)
        golden[1 + 2 * i] = np.float16(max_index.view(np.float16))
