...
## 算子描述
- 算子功能：  
  ReduceMinCustom算子在所有的输入tensor的的前n个数据中找出最小值及最小值对应的索引位置。
- 算子规格：  
  <table>
  <tr>
...
- 算子实现：  
  ...
  - Kernel实现
    ...
    ReduceMinCustom算子的实现流程分为3个基本任务：CopyIn，Compute，CopyOut。CopyIn任务负责将Global Memory上的输入Tensor srcGm存储在srcLocal中。Compute任务负责从srcLocal的前288个数据中找出最小值及最小值对应的索引位置，并将结果存储在dstLocal中。CopyOut任务负责将输出数据从dstLocal搬运至Global Memory上的输出Tensor dstGm。
