# load_data_with_transpose_b16样例
## 概述
本样例介绍基础API LoadDataWithTranspose b16数据类型下的使用样例。
...
│   └── load_data_with_transpose_b16.asc            // Ascend C算子实现 & 调用样例
```

## 算子描述
- 算子功能：  
  本样例中实现的是[m, n, k]固定为[16, 32, 32]的Matmul算子，并使用Ascend C基础Api实现。对应的数学表达式为：
  ```
  C = A * B
  ```
- 算子规格：
  ...
  <tr><td rowspan="1" align="center">核函数名</td><td colspan="4" align="center">load_data_with_transpose_b16</td></tr>
  </table>
- 算子实现：  
  Mmad算子的数学表达式为：
  ```
