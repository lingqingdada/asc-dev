...
class MicroReduceMinTest {
public:
    __aicore__ inline MicroReduceMinTest() {}
    ...
    __aicore__ inline void Compute()
    {
        ...
        __VEC_SCOPE__
        {
            ...
            for (uint16_t i = 0; i < static_cast<uint16_t>(repeatTimes); ++i) {
                ...
                Reg::ReduceMin(vDstReg, vSrcReg, preg);
...
template <typename T, int mode>
__global__ __aicore__ void testReduceMin(__gm__ uint8_t* dstGm, __gm__ uint8_t* srcGm,
    __gm__ uint8_t* indexOffsetGm, uint32_t elementCount)
{
    AscendC::MicroReduceMinTest<T, mode> op;
...
class reduceMinTestsuite : public testing::Test,
    public testing::WithParamInterface<ReduceMinParams> {
protected:
    void SetUp() {
...
INSTANTIATE_TEST_CASE_P(TEST_OPEARATION_REDUCE_MIN, reduceMinTestsuite,
    ::testing::Values(ReduceMinParams { 2, 2, 128, testReduceMin<half, 0>},
    ReduceMinParams { 4, 4, 64, testReduceMin<int32_t, 0>},
    ReduceMinParams { 4, 4, 64, testReduceMin<uint32_t, 0>},
    ReduceMinParams { 4, 4, 64, testReduceMin<int16_t, 0>},
    ReduceMinParams { 4, 4, 64, testReduceMin<uint16_t, 0>},
    ReduceMinParams { 4, 4, 64, testReduceMin<float, 0>}));


TEST_P(reduceMinTestsuite, testReduceMin)
{
    auto param = GetParam();
