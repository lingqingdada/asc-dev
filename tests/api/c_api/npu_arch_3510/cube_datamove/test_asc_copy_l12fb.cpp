...
void copy_cbuf_to_fbuf_size_Stub(__fbuf__ void* dst_in, __cbuf__ void* src_in, uint16_t burst_num_in, uint16_t burst_len_in,
                                      uint16_t src_gap_size_in, uint16_t dst_gap_size_in)
{
    __fbuf__ void* dst = reinterpret_cast<__fbuf__ void*>(1);
    __cbuf__ void* src = reinterpret_cast<__cbuf__ void*>(2);
...
TEST_F(TestCubeDmamoveCopyL12FbCAPI, c_api_asc_copy_l12fb_size_Succ)
{
    __fbuf__ void* dst = reinterpret_cast<__fbuf__ void*>(1);
    __cbuf__ void* src = reinterpret_cast<__cbuf__ void*>(2);
    uint32_t size = 44;
    ...
    MOCKER(copy_cbuf_to_fbuf, void(__fbuf__ void*, __cbuf__ void*, uint16_t, uint16_t, uint16_t, uint16_t))
        .times(1)
        .will(invoke(copy_cbuf_to_fbuf_size_Stub));
    
    asc_copy_l12fb(dst, src, size);
    GlobalMockObject::verify();
}