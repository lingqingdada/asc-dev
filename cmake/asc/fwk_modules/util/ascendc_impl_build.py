            ...
            for sub_sections in src_ini_config.sections():
                if sub_sections[65:] == self.op_type:
                    if src_ini_config.has_option(sub_sections, "kernel_dir"):
                        src_file_path = os.path.join(impl_path, \
src_ini_config.get(sub_sections, "kernel_dir"), src_ini_config.get(sub_sections, "kernel_file"))
                    else:
        ...
        if os.environ.get('BUILD_BUILTIN_OPP') != '1' and impl_path != "":
            ...
            if not os.path.exists(src_file):
                raise FileNotFoundError(f"operator: {self.op_file} source file does not found, "
    ...
    def _get_impl_mode(self: any):
        ...
        if "impl_mode" in op_compile_options:
            ...
            self.impl_mode_op_info = f', impl_mode = impl_mode'
            ...
            for sub_sections in src_ini_config.sections():
                if sub_sections[65:] == self.op_type:
                    ...
                    if len(src_compute_units_low) == 0:
                        if src_ini_config.has_option(sub_sections, "kernel_dir"):
                            src_file_dict["__ALLSOC__"] = src_ini_config.get(sub_sections, "kernel_dir") + "/" + \
    ...
    def _write_impl(self: any, fd: object, impl_path: str = "", src_ini_config=None):
        ...
        fd.write(IMPL_API.format(self.op_type, pchk, \
                                 ...
                                 optype_snake_ex(self.op_type), optype_snake(self.op_type), src_file_dict, src))
...
def write_scripts(cfgfile: str, cfgs: dict, dirs: dict, ops: list = None, \
    op_compile_option:list = None, kernel_source_ini_file: str = None):
    ...
    for op_desc in op_descs:
        op_desc.write_adapt(dirs.get(const_var.CFG_IMPL_DIR), \
            dirs.get(const_var.CFG_OUT_DIR), op_compile_option, src_ini_config)
