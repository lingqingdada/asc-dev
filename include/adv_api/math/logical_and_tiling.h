...
/*!
 * \file logical_and_tiling.h
 * \brief
 */

#if !defined(__ASCENDC_INCLUDE_INTERNAL_HEADERS__)
#define __ASCENDC_INCLUDE_INTERNAL_HEADERS__
#define __UNDEF_ASCENDC_INCLUDE_INTERNAL_HEADERS_LOGICAL_AND_TILING_H__
#endif

#ifndef LIB_MATH_LOGICAL_AND_TILING_H
#define LIB_MATH_LOGICAL_AND_TILING_H
 ...
 */
void GetLogicalAndMaxMinTmpSize(const platform_ascendc::PlatformAscendC& ascendcPlatform, const ge::Shape& srcShape, 
    const uint32_t typeSize, const bool isReuseSource, uint32_t& maxValue, uint32_t& minValue);

/*!
 * \brief The calculation of the LogicalAnd interface requires the developer to reserve or apply for temporary space.
 ...
 */
void GetLogicalAndTmpBufferFactorSize(const platform_ascendc::PlatformAscendC& ascendcPlatform, const uint32_t typeSize, 
    uint32_t& maxLivedNodeCount, uint32_t& extraBuf);
}  // namespace AscendC
#endif  // LIB_MATH_LOGICAL_AND_TILING_H

#if defined(__UNDEF_ASCENDC_INCLUDE_INTERNAL_HEADERS_LOGICAL_AND_TILING_H__)
#undef __ASCENDC_INCLUDE_INTERNAL_HEADERS__
#undef __UNDEF_ASCENDC_INCLUDE_INTERNAL_HEADERS_LOGICAL_AND_TILING_H__
#endif