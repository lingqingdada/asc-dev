...
#define __UNDEF_ASCENDC_INCLUDE_INTERNAL_HEADERS_FMA_TILING_H__
#endif
 ...
 */
void GetFmaMaxMinTmpSize(const platform_ascendc::PlatformAscendC& ascendcPlatform, const ge::Shape& srcShape, 
    const uint32_t typeSize, const bool isReuseSource, uint32_t& maxValue, uint32_t& minValue);

/*!
 * \brief The calculation of the Fma interface requires the developer to reserve or apply for temporary space.
 ...
 */
void GetFmaTmpBufferFactorSize(const platform_ascendc::PlatformAscendC& ascendcPlatform, const uint32_t typeSize, 
    uint32_t& maxLivedNodeCount, uint32_t& extraBuf);
} // namespace AscendC
#endif // LIB_MATH_FMA_TILING_H

#if defined(__UNDEF_ASCENDC_INCLUDE_INTERNAL_HEADERS_FMA_TILING_H__)
#undef __ASCENDC_INCLUDE_INTERNAL_HEADERS__
#undef __UNDEF_ASCENDC_INCLUDE_INTERNAL_HEADERS_FMA_TILING_H__
#endif