...
DataCopyPad接口提供非对齐搬运的功能，如果基于该接口支持的产品开发算子（参见产品支持情况），则可以直接使用该接口解决非对齐场景下的搬运问题。使用DataCopyPad的完整示例请参考[DataCopyPad样例](https://gitcode.com/cann/asc-devkit/tree/master/examples/01_simd_cpp_api/02_features/03_basic_api/00_data_movement/data_copy_pad_gm2ub_ub2gm)。
...
-   样例一：[冗余数据参与计算+使用GatherMask收集有效数据后搬出](https://gitcode.com/cann/asc-devkit/blob/master/examples/01_simd_cpp_api/02_features/03_basic_api/02_memory_vector_compute/abs_gather_mask)。
...
-   <a name="zh-cn_topic_0000001945665465_li1891184254913"></a>样例二：[通过Duplicate逐行清零+带冗余数据搬出](https://gitcode.com/cann/asc-devkit/tree/master/examples/01_simd_cpp_api/02_features/03_basic_api/02_memory_vector_compute/abs_duplicate)。
    ...
    size_t inputByteSize = elemLength * sizeof(int16_t);
    // copy out atomic add extra (BLOCKLEN_CEIL - BLOCK_ELEMENT_NUM) zeros to dstGM
    size_t outputByteSize = elemLength * sizeof(int16_t);
    ```

-   样例三：[冗余数据参与计算+使用UnPad接口去除冗余数据后搬出](https://gitcode.com/cann/asc-devkit/tree/master/examples/01_simd_cpp_api/02_features/03_basic_api/02_memory_vector_compute/abs_unpad)。
...
-   示例四：[通过Pad一次性清零+带冗余数据搬出](https://gitcode.com/cann/asc-devkit/tree/master/examples/01_simd_cpp_api/02_features/03_basic_api/02_memory_vector_compute/abs_pad)。
...
-   样例五：[使用mask掩掉冗余数据+带冗余数据搬出](https://gitcode.com/cann/asc-devkit/tree/master/examples/01_simd_cpp_api/02_features/03_basic_api/02_memory_vector_compute/reduce_min)。
