...
#pragma message("impl/basic_api/dav_l300/kernel_operator_vec_gather_mask_impl.h is an internal header file and must not be used directly. Functions or variables defined in this file may be removed in the future. Please use \"#include \"basic_api/kernel_vec_intf.h\"\" and use public functions or variables defined in interface headers files.")
...
{
    ASSERT(false && "unsupported data type in gatherMask");
}
...
#define REGISTER_GATHER_MASK_B16(data_type, vector_type)                                                               \
    ...
    {                                                                                                                  \
        ...
        uint16_t loopNum = (uint16_t)reducev2Params.repeatTimes;                                                       \
        if (reduceMode) {                                                                                              \
            __VEC_SCOPE__ {                                                                                            \
                vector_align ureg0;                                                                                    \
...
__aicore__ inline void GatherMaskCal(__ubuf__ T* dst, __ubuf__ T* src0, const uint8_t src1Pattern,
    const bool reduceMode, const uint32_t mask, const GatherMaskParams& reducev2Params, uint64_t& rsvdCnt)
{
    ASSERT(false && "unsupported Gather.");
}
