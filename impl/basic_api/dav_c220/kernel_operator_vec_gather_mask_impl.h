...
#pragma message("impl/basic_api/dav_c220/kernel_operator_vec_gather_mask_impl.h is an internal header file and must not be used directly. Functions or variables defined in this file may be removed in the future. Please use \"#include \"basic_api/kernel_vec_intf.h\"\" and use public functions or variables defined in interface headers files.")
    ...
    const GatherMaskParams& gatherMaskParams)
{
    if ASCEND_IS_AIV {
        ASCENDC_DEBUG_ASSERT((SupportType<T, half, bfloat16_t, uint16_t, int16_t, float, uint32_t, int32_t>()),
            KERNEL_LOG_INTERNAL(KERNEL_ERROR, "Failed to check dtype in GatherMask, current api support dtype "
            "combination is src0 and dst both: half / bfloat16_t / uint16_t / int16_t / float / uint32_t / int32_t.\n"));
        if (sizeof(T) == sizeof(uint16_t)) {
            vreducev2(reinterpret_cast<__ubuf__ uint16_t*>(dst), reinterpret_cast<__ubuf__ uint16_t*>(src0),
                reinterpret_cast<__ubuf__ uint16_t*>(src1), gatherMaskParams.repeatTimes, gatherMaskParams.src0BlockStride,
            ...
            vreducev2(reinterpret_cast<__ubuf__ uint32_t*>(dst), reinterpret_cast<__ubuf__ uint32_t*>(src0),
                reinterpret_cast<__ubuf__ uint32_t*>(src1), gatherMaskParams.repeatTimes, gatherMaskParams.src0BlockStride,
                patternMode, gatherMaskParams.src0RepeatStride, gatherMaskParams.src1RepeatStride);
        }
    }
}
...
template <typename T>
__aicore__ inline void GatherMaskImpl(__ubuf__ T* dst, __ubuf__ T* src0, const uint8_t patternMode,
    const GatherMaskParams& gatherMaskParams)
{
#if defined(ASCENDC_DEBUG) || defined(ASCENDC_CPU_DEBUG)
    ...
    const uint32_t mask, const GatherMaskParams& gatherMaskParams, uint64_t& rsvdCnt)
{
    ...
    if constexpr(SupportType<U, uint16_t>()) {
        ASCENDC_DEBUG_ASSERT((SupportType<T, half, bfloat16_t, uint16_t, int16_t>()), KERNEL_LOG_INTERNAL(KERNEL_ERROR,
        ...
        ASCENDC_DEBUG_ASSERT((SupportType<T, float, uint32_t, int32_t>()), KERNEL_LOG_INTERNAL(KERNEL_ERROR,
...
#endif
    ASCENDC_DEBUG_ASSERT((SupportType<T, half, bfloat16_t, uint16_t, int16_t, float, uint32_t, int32_t>()),
        KERNEL_LOG_INTERNAL(KERNEL_ERROR, "Failed to check dtype in GatherMask, current api support dtype combination "
