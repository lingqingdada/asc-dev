...
#pragma message("impl/basic_api/dav_m200/kernel_operator_vec_transpose_impl.h is an internal header file and must not be used directly. Functions or variables defined in this file may be removed in the future. Please use \"#include \"basic_api/kernel_tensor.h\"\" and use public functions or variables defined in interface headers files.")
    ...
    const LocalTensor<uint8_t> &sharedTmpBuffer, const TransposeParamsExt &transposeParams)
{
    ...
    if (transposeParams.transposeType == TransposeType::TRANSPOSE_NCHW2NHWC) {
        for (int i = 0; i < transposeParams.nSize; i++) {
            v4dtrans((__ubuf__ U*)dst[channelImageSize * i].GetPhyAddr(),
                (__ubuf__ U*)src[channelImageSize * i].GetPhyAddr(), imageSize, transposeParams.cSize, false);
        }
    } else if (transposeParams.transposeType == TransposeType::TRANSPOSE_NHWC2NCHW) {
        for (int i = 0; i < transposeParams.nSize; i++) {
            v4dtrans((__ubuf__ U*)dst[channelImageSize * i].GetPhyAddr(),
                (__ubuf__ U*)src[channelImageSize * i].GetPhyAddr(), imageSize, transposeParams.cSize, true);
        }
    }
}
} // namespace AscendC
