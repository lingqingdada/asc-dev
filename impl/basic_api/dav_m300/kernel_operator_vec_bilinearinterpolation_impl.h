...
#pragma message("impl/basic_api/dav_m300/kernel_operator_vec_bilinearinterpolation_impl.h is an internal header file and must not be used directly. Functions or variables defined in this file may be removed in the future. Please use \"#include \"basic_api/kernel_tensor.h\"\" and use public functions or variables defined in interface headers files.")
...
__aicore__ inline void BilinearInterpolationCalc(LocalTensor<T> &dst, LocalTensor<T> &src0,
    LocalTensor<uint32_t> &src0Offset, LocalTensor<T> &src1, uint64_t mask[], uint8_t hRepeat,
    bool repeatMode, uint16_t dstBlkStride, uint16_t vROffset, uint8_t vRepeat,
    const LocalTensor<uint8_t> &sharedTmpBuffer)
{
    ASCENDC_REPORT_NOT_SUPPORT(false, "BilinearInterpolation");
