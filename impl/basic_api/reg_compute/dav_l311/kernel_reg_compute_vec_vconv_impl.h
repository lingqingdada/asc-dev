...
__simd_callee__ inline void TruncateImpl(U& dstReg, U& srcReg, MaskReg& mask)
{
    ...
    static_assert(SupportType<ActualT, half, float>(), "current trunc data type is not supported on current device!");
