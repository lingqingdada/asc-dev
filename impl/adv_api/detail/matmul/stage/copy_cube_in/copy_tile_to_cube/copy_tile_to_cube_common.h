...
private:
        ...
        int32_t orgHeight, int32_t orgWidth, int32_t stepCol, bool iskRowDirec)
    {
        ...
        if (tileWidth < baseWidth || baseWidth % B8_C0SIZE == 0 || stepCol == 1 ||
            ...
            if (likely(dstNzMatrixStride <= UINT16_MAX)) {
                ...
                PadZeroForL1(dst, tileHeight, tileWidth, baseHeight, baseWidth, CeilAlign(baseWidth, B8_C0SIZE) * stepCol);
            } else {
                ...
                for (int i = 0; i < stepCol; ++i) {
                    if (i == stepCol - 1) {
                        baseWidth = (tileWidth - (stepCol - 1) * baseWidth) == 0 ? baseWidth: tileWidth - (stepCol - 1) * baseWidth;
                    }
    ...
    __aicore__ inline void PadZeroForL1(const LocalTensor<TransT>& dst, int32_t tileHeight, int32_t tileWidth, int32_t baseHeight, int32_t baseWidth,
        int32_t padZeroSize)
    {
    ...
    template <typename T>
    __aicore__ inline void PadZerosForOuterAxis(const LocalTensor<T>& dst, const int32_t height,
        const int32_t width, const int32_t baseHeight, const int32_t baseWidth)
    {
        if constexpr (HasScalePosition<INPUT_TYPE>::value) {
    ...
    template <typename T>
    __aicore__ inline void PadOneBlockInnerAxis(const LocalTensor<T>& dst, const int32_t height,
        const int32_t width)
    {
        ...
        int32_t orgHeight, int32_t orgWidth, int32_t stepCol, bool iskRowDirec)
    {
        if (tileWidth < baseWidth || baseWidth % c0Size_ == 0 || stepCol == 1) {
            MATMUL_MODULE(DataCopyWrapper)->CopyDN2NZ(dst, src, curRow * baseHeight, curCol * baseWidth, tileHeight,
                tileWidth, orgWidth, 1, 0, 0, iskRowDirec);
        } else {
            if ((stepCol - 1) * baseWidth > tileWidth) {
                stepCol = Ceil(tileWidth, baseWidth);
            ...
            if (likely(dstNzMatrixStride <= UINT16_MAX)) {
                MATMUL_MODULE(DataCopyWrapper)->CopyDN2NZ(dst, src, curRow * baseHeight,
                    curCol * baseWidth, tileHeight, baseWidth, orgWidth, stepCol - 1, baseWidth,
                    dstNzMatrixStride, iskRowDirec);
                MATMUL_MODULE(DataCopyWrapper)->CopyDN2NZ(dst[(stepCol - 1) * dstNzMatrixStride], src,
                    curRow * baseHeight, (curCol + stepCol - 1) * baseWidth, tileHeight,
                    tileWidth - (stepCol - 1) * baseWidth, orgWidth, 1, 0, 0, iskRowDirec);
            } else {
                int32_t colIndex = curCol * baseWidth;
                ...
                for (int i = 0; i < stepCol; ++i) {
                    ...
                    MATMUL_MODULE(DataCopyWrapper)->CopyDN2NZ(dst[dstOffset], src, curRow * baseHeight, colIndex, tileHeight,
                        baseWidth, orgWidth, 1, 0, 0, iskRowDirec);
                    colIndex += baseWidth;
                    dstOffset += dstNzMatrixStride;
                }
    ...
    __aicore__ inline void GetQtable(uint64_t &qtable0, uint64_t &qtable1, int32_t curRow, 
        int32_t curCol, int32_t baseHeight) 
    {
        uint16_t qtableIndex =
            curRow + curCol * Ceil(MATMUL_MODULE(CopyCubeInParams)->GetSingleHeight(), baseHeight);
        ...
        int32_t orgHeight, int32_t orgWidth, int32_t stepCol, bool iskRowDirec)
    {
        if constexpr (INPUT_TYPE::format == CubeFormat::ND) {
            ...
            if constexpr (sizeof(TransT) == sizeof(int8_t)) {
                ...
                MATMUL_MODULE(DataCopyWrapper)->CopyDN2NZ(dst, src, curRow * baseHeight, curCol * baseWidth,
                    tileHeight, tileWidth, orgWidth);
            }
#endif
            ...
            if constexpr (IsDecompMode<MM_CFG>() && INPUT_TYPE::TAG == InputTypeTag::B) {
                ...
                MATMUL_MODULE(DataCopyWrapper)->CopyNZ2NZDecompMode(dst, src,
                    curRow * baseHeight, curCol * baseWidth, tileHeight, tileWidth, orgHeight, qtable0, qtable1, iskRowDirec);
            } else {
            ...
            ASCENDC_ASSERT(false,
                { KERNEL_LOG(KERNEL_ERROR, "Unsupported Matmul format type."); });
        }
    }
        ...
        int32_t curRow, int32_t curCol, int32_t tileHeight, int32_t tileWidth, int32_t widthFactor)
    {
        ...
        if constexpr (INPUT_TYPE::format == CubeFormat::ND) {
            MATMUL_MODULE(DataCopyWrapper)->CopyND2NZ(dst, src, curRow * baseHeight, curCol * baseWidth, tileHeight, tileWidth, orgWidth, iskRowDirec);
        } else if constexpr (INPUT_TYPE::format == CubeFormat::NZ) {
...
#if defined(__UNDEF_ASCENDC_INCLUDE_INTERNAL_HEADERS_DETAIL_MATMUL_STAGE_COPY_CUBE_IN_COPY_TILE_TO_CUBE_COPY_TILE_TO_CUBE_COMMON_H__)
