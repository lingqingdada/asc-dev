...
#define UNDEF_ASCENDC_INCLUDE_INTERNAL_HEADERS_SOFTMAX_FLASH_IMPL_H
#endif

#ifndef IMPL_ACTIVATION_SOFTMAX_L300_SOFTMAX_FLASH_IMPL_H
#define IMPL_ACTIVATION_SOFTMAX_L300_SOFTMAX_FLASH_IMPL_H
    ...
    const uint16_t repeatTimes, const uint32_t srcK)
{
    ...
    for (uint16_t i = 0; i < srcM; ++i) {
        ...
        if constexpr (sizeof(T2) == sizeof(half)) {
            Reg::DataCopy(tmpUb + i * blockStride, sumVreg, maskOneBlk);
        }
    }
    ...
    for (uint16_t i = 0; i < srcM; ++i) {
        ...
        for (uint16_t j = 0; j < repeatTimes; ++j) {
            Reg::DataCopy(tmpVreg, workUb + i * srcK + j * stride);
...
#if defined(UNDEF_ASCENDC_INCLUDE_INTERNAL_HEADERS_SOFTMAX_FLASH_IMPL_H)
#undef _ASCENDC_INCLUDE_INTERNAL_HEADERS_
#undef UNDEF_ASCENDC_INCLUDE_INTERNAL_HEADERS_SOFTMAX_FLASH_IMPL_H
#endif
