...
#pragma message("impl/adv_api/detail/activation/softmax/regbase/l300/softmax_flashv2_impl.h is an internal header file and must not be used directly. Functions or variables defined in this file may be removed in the future. Please use \"#include \"adv_api/activation/softmaxflashv2.h\"\" and use public functions or variables defined in interface headers files.")
#define _ASCENDC_INCLUDE_INTERNAL_HEADERS_
#define UNDEF_ASCENDC_INCLUDE_INTERNAL_HEADERS_SOFTMAX_FLASHV2_IMPL_H
#endif

#ifndef IMPL_ACTIVATION_SOFTMAX_L300_SOFTMAX_FLASHV2_IMPL_H
#define IMPL_ACTIVATION_SOFTMAX_L300_SOFTMAX_FLASHV2_IMPL_H
    ...
    __ubuf__ float* expUb, const LastAxisShapeND originalSrcShape, const SoftMaxTiling tiling)
{
    ...
    for (uint16_t j = 0; j < kRepeatTimes; ++j) {
        ...
        for (uint16_t i = 0; i < mRepeatTimes; ++i) {
            ...
            if constexpr (sizeof(T1) == 2) {
                Reg::DataCopy(expUb + i * FLOAT_REPEAT_SIZE + j * dataBlock, tmpVreg, pregCnt);
            }
        }
    }
    ...
    for (uint16_t i = 0; i < mRepeatTimes; ++i) {
        ...
        for (uint16_t j = 0; j < kRepeatTimes; ++j) {
            if constexpr (sizeof(T1) == 2) {
                Reg::DataCopy(srcVreg, expUb + i * FLOAT_REPEAT_SIZE + j * dataBlock);
            } else {
                Reg::DataCopy(srcVreg, dstUb + i * FLOAT_REPEAT_SIZE + j * dataBlock);
            }
    ...
    for (uint16_t i = 0; i < e2bRep; ++i) {
        pregCnt = Reg::UpdateMask<T2>(sreg);
        LoadE2B<T2>(castReg, tmpUb + i * DEFAULT_BLK_NUM);
        Reg::DataCopy(expSumUb + i * dtypeRepStride, castReg, pregCnt);
    }
}
    ...
    __ubuf__ float* expUb, const LastAxisShapeND originalSrcShape, const SoftMaxTiling tiling)
{
    ...
    for (uint16_t i = 0; i < mRepeatTimes; ++i) {
        ...
        if constexpr (sizeof(T1) == 2) {
            Reg::DataCopy(expUb + i * FLOAT_REPEAT_SIZE + kRepeatTimes * dataBlock, tmpVreg, pregDst);
        }
    }
    ...
    for (uint16_t i = 0; i < mRepeatTimes; ++i) {
        ...
        if constexpr (sizeof(T1) == 2) {
            Reg::DataCopy(srcVreg, expUb + i * FLOAT_REPEAT_SIZE + kRepeatTimes * dataBlock);
        } else {
            Reg::DataCopy(srcVreg, dstUb + i * FLOAT_REPEAT_SIZE + kRepeatTimes * dataBlock);
        }
        Reg::Select(srcVreg, srcVreg, minVreg, pregkTail);
        Reg::Add(sumVreg, sumVreg, srcVreg, pregFull);
    ...
    const LastAxisShapeND originalSrcShape, const SoftMaxTiling tiling)
{
    ...
    for (uint16_t i = 0; i < VcgFoldRepeat; ++i) {
        ...
        if constexpr (sizeof(T2) == 4) {
            Reg::DataCopy(workUb + i * FLOAT_REPEAT_SIZE, maxVreg, pregFull);
        } else {
            Reg::DataCopy<float, Reg::StoreDist::DIST_INTLV_B32>(
                workUb + i * HALF_REPEAT_SIZE, maxVreg, maxVreg, pregFull);
        }
    ...
    for (uint16_t i = 0; i < e2bRep; ++i) {
        ...
        if constexpr (sizeof(T2) == 4) {
            ...
            Reg::DataCopy(tmpUb + i * FLOAT_REPEAT_SIZE, maxVreg, pregFull);
        }
        Reg::FusedExpSub(dstVreg, inMaxVreg, maxVreg, pregCnt);
        Reg::DataCopy(expMaxF32Ub + i * FLOAT_REPEAT_SIZE, dstVreg, pregCnt);
    }
    ...
    for (uint16_t j = 0; j < kRepeatTimes; ++j) {
        ...
        for (uint16_t i = 0; i < mRepeatTimes; ++i) {
            pregCnt = Reg::UpdateMask<uint32_t>(sreg);
            Reg::DataCopy(maxVreg, tmpUb + i * FLOAT_REPEAT_SIZE);
            LoadIfNeedCast<T1>(srcVreg, srcUb + i * FLOAT_REPEAT_SIZE + j * dataBlock, pregFull);
            Reg::FusedExpSub(tmpVreg, srcVreg, maxVreg, pregCnt);
            StoreIfNeedCast<T1>(dstUb + i * FLOAT_REPEAT_SIZE + j * dataBlock, tmpVreg, pregCnt);
            if constexpr (sizeof(T1) == 2) {
    ...
    for (uint16_t i = 0; i < mRepeatTimes; ++i) {
        ...
        for (uint16_t j = 0; j < kRepeatTimes; ++j) {
            if constexpr (sizeof(T1) == 2) {
                ...
                Reg::DataCopy(tmpVreg, dstUb + i * FLOAT_REPEAT_SIZE + j * dataBlock);
            }
            Reg::Add(sumVreg, sumVreg, tmpVreg, pregFull);
        }
        Reg::ReduceSumWithDataBlock(sumVreg, sumVreg, pregFull);
    ...
    for (uint16_t i = 0; i < VcgFoldRepeat; ++i) {
        ...
        Reg::Add(sumVreg, sumVreg, tmpVreg, pregFull);
        if constexpr (sizeof(T2) == 4) {
            Reg::DataCopy(workUb + i * FLOAT_REPEAT_SIZE, sumVreg, pregFull);
        } else {
            Reg::DataCopy<float, Reg::StoreDist::DIST_INTLV_B32>(
    ...
    for (uint16_t i = 0; i < e2bRep; ++i) {
        ...
        Reg::DataCopy(maxVreg, expMaxF32Ub + i * FLOAT_REPEAT_SIZE);
        ...
        if constexpr (sizeof(T1) == 2 && sizeof(T2) == 4) {
            ...
            Reg::Pack<uint16_t, uint32_t>((Reg::RegTensor<uint16_t> &)t1Reg,
                (Reg::RegTensor<uint32_t> &)t1Reg);
            Reg::DataCopy<T1, Reg::StoreDist::DIST_INTLV_B16>(
    ...
    const LastAxisShapeND originalSrcShape, const SoftMaxTiling tiling)
{
    ...
    for (uint16_t i = 0; i < mRepeatTimes; ++i) {
        ...
        if constexpr (sizeof(T1) == 2) {
            ...
            Reg::DataCopy(tmpVreg, dstUb + i * FLOAT_REPEAT_SIZE + kRepeatTimes * dataBlock);
        }
...
#endif // IMPL_ACTIVATION_SOFTMAX_L300_SOFTMAX_FLASHV2_IMPL_H

#if defined(UNDEF_ASCENDC_INCLUDE_INTERNAL_HEADERS_SOFTMAX_FLASHV2_IMPL_H)
#undef _ASCENDC_INCLUDE_INTERNAL_HEADERS_
#undef UNDEF_ASCENDC_INCLUDE_INTERNAL_HEADERS_SOFTMAX_FLASHV2_IMPL_H
#endif
