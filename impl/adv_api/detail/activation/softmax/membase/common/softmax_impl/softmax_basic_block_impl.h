...
__aicore__ inline void SoftMaxBasicBlock(
    ...
    for (uint32_t i = 0; i < tiling.rangeM; i++) {
        ...
        if (splitBlock == 1) {
            BlockReduceMax<float, false>(
                tmpBuffer1, tmpBuffer0, (uint8_t)(tiling.splitM), MASK_PLACEHOLDER, 1, 1, DEFAULT_REPEAT_STRIDE);
        } else {
        ...
        for (uint32_t j = 0; j < splitCeilM; j++) {
            AlignedBrcbImpl(
                tmpBuffer1[FLOAT_REPEAT_SIZE * j * HALF_FACTOR], reduceSumBuffer[FLOAT_NUM_PER_BLK * j],
        ...
        Brcb(
            tmpBuffer1[DEFAULT_BLK_NUM], reduceSumBuffer, splitCeilM,
        ...
        if (splitBlock == 1) {
            BlockReduceSum<float, false>(
                tmpBuffer1, tmpBuffer0, (uint8_t)(tiling.splitM), MASK_PLACEHOLDER, 1, 1, DEFAULT_BLK_NUM);
        } else {
        ...
        PipeBarrier<PIPE_V>();
        Cast<half, float, false>(
            sumTensor[offset2], tmpBuffer1, FLOAT2HALF_ROUND_MODE, MASK_PLACEHOLDER, reduceCeilValue,
            {1, 1, HALF_REPEAT_STRIDE, DEFAULT_REPEAT_STRIDE});
    ...
    const LocalTensor<float>& src, const LocalTensor<float>& workLocal, const SoftMaxTiling& tiling)
{
    ...
    for (uint32_t i = 0; i < tiling.rangeM; i++) {
        ...
        if (splitBlock == 1) {
            BlockReduceMax<float, false>(
                tmpBuffer1, src[offset1], (uint8_t)(tiling.splitM), MASK_PLACEHOLDER, 1, 1, DEFAULT_REPEAT_STRIDE);
        } else {
        ...
        if (splitBlock == 1) {
            BlockReduceSum<float, false>(
                tmpBuffer1, dst[offset1], (uint8_t)(tiling.splitM), MASK_PLACEHOLDER, 1, 1, DEFAULT_REPEAT_STRIDE);
        } else {
        ...
        for (uint32_t j = 0; j < splitCeilM; j++) {
            AlignedBrcbImpl(
                sumTensor[offset2 + FLOAT_REPEAT_SIZE * j], tmpBuffer2[FLOAT_NUM_PER_BLK * j], FLOAT_NUM_PER_BLK);
        }
        ResetMask();
#elif defined(__NPU_ARCH__) && __NPU_ARCH__ == 2201
        ...
        for (uint32_t j = 0; j < splitBlock; ++j) {
            Div<float, false>(
                dst[offset1 + FLOAT_REPEAT_SIZE * j], dst[offset1 + FLOAT_REPEAT_SIZE * j], sumTensor[offset2],
                MASK_PLACEHOLDER, (uint8_t)(tiling.splitM), {1, 1, 0, offset, offset, 1});
        }
        PipeBarrier<PIPE_V>();
    }
    ...
    const LocalTensor<half>& src, const LocalTensor<float>& workLocal, const SoftMaxTiling& tiling)
{
    ...
    for (uint32_t i = 0; i < tiling.rangeM; i++) {
        ...
        PipeBarrier<PIPE_V>();
        BlockReduceMax<float, false>(
            reduceSumBuffer, tmpBuffer1, splitCeilM, MASK_PLACEHOLDER, 1, 1, DEFAULT_REPEAT_STRIDE);
        PipeBarrier<PIPE_V>();
#if defined(__NPU_ARCH__) && __NPU_ARCH__ == 2002
        event_t eventIdVToS = static_cast<event_t>(GetTPipePtr()->FetchEventID(HardEvent::V_S));
        SetFlag<HardEvent::V_S>(eventIdVToS);
        ...
        for (uint32_t j = 0; j < splitCeilM; j++) {
            AlignedBrcbImpl(
                maxTensor[offset2 + FLOAT_REPEAT_SIZE * j], reduceSumBuffer[FLOAT_NUM_PER_BLK * j], FLOAT_NUM_PER_BLK);
        }
        ResetMask();
#elif defined(__NPU_ARCH__) && __NPU_ARCH__ == 2201
        ...
        for (uint32_t j = 0; j < splitBlock; ++j) {
            Sub<float, false>(
                tmpBuffer0[FLOAT_REPEAT_SIZE * j], tmpBuffer0[FLOAT_REPEAT_SIZE * j], maxTensor[offset2],
                MASK_PLACEHOLDER, (uint8_t)(tiling.splitM), {1, 1, 0, offset, offset, 1});
        }
        PipeBarrier<PIPE_V>();
        Exp<float, false>(
        ...
        for (uint32_t j = 0; j < splitCeilM; j++) {
            AlignedBrcbImpl(
                sumTensor[offset2 + FLOAT_REPEAT_SIZE * j], reduceSumBuffer[FLOAT_NUM_PER_BLK * j], FLOAT_NUM_PER_BLK);
        }
        ResetMask();
#elif defined(__NPU_ARCH__) && __NPU_ARCH__ == 2201
        ...
        for (uint32_t j = 0; j < splitBlock; ++j) {
            Div<float, false>(
                tmpBuffer0[FLOAT_REPEAT_SIZE * j], tmpBuffer0[FLOAT_REPEAT_SIZE * j], sumTensor[offset2],
                MASK_PLACEHOLDER, (uint8_t)(tiling.splitM), {1, 1, 0, offset, offset, 1});
        }
        PipeBarrier<PIPE_V>();
        Cast<half, float, false>(
