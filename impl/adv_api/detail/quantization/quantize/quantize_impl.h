...
template <typename T>
__simd_callee__ inline void DuplicateScalarToFloatVector(Reg::RegTensor<float>& floatVreg, const T& scalar,
                                                    Reg::RegTensor<T>& tempVreg, Reg::MaskReg& mask)
{
    if constexpr (SupportType<T, half, bfloat16_t>()) {
...
__simd_callee__ inline void CastFp32DstToExpect(Reg::RegTensor<float>& srcVreg, Reg::RegTensor<DstT>& dstVreg,
    Reg::MaskReg& mask)
{
...
__aicore__ inline void QuantizePerGroupForKColCommon(const LocalTensor<DstT>& dstTensor,
    const LocalTensor<SrcT>& srcTensor, const ScaleT& scale, const OffsetT& offset, const QuantizeParams& params)
{
    ...
    for (uint16_t i = 0; i < rowNum; ++i) {
        ...
        for (uint16_t j = 0; j < repeat; ++j) {
            ...
            Reg::StoreAlign<uint8_t, Reg::StoreDist::DIST_PACK4_B32>(
                ...
                (Reg::RegTensor<uint8_t> &)dstVreg, mask);
        }
    }
}

template <const QuantizeConfig& config, typename DstT, typename SrcT, typename ScaleT>
__aicore__ inline void QuantizePerGroupForKColFp4(const LocalTensor<DstT>& dstTensor,
    const LocalTensor<SrcT>& srcTensor, const ScaleT& scale, const QuantizeParams& params)
{
    using ActualScaleT = typename ScaleT::PrimType;
    __ubuf__ DstT* dstUb = (__ubuf__ DstT*)dstTensor.GetPhyAddr();
    __ubuf__ SrcT* srcUb = (__ubuf__ SrcT*)srcTensor.GetPhyAddr();
...
template <typename DstT, typename SrcT, typename ActualScaleT, bool hasOffset, bool isScalarOffset, const Reg::CastTrait& castTrait>
__simd_callee__ inline void QuantizePerGroupForKRowCommonOneRow(__ubuf__ DstT *dstAddr, __ubuf__ SrcT *srcAddr,
    __ubuf__ ActualScaleT *scaleAddr, __ubuf__ ActualScaleT *offsetAddr, Reg::RegTensor<DstT> &dstVreg,
    Reg::RegTensor<float> &srcVreg, Reg::RegTensor<float> &scaleVreg,
    Reg::RegTensor<float> &offsetVreg,  uint16_t repeat, uint32_t n, uint32_t vecLen)
{
    ...
    for (uint16_t j = 0; j < repeat; ++j) {
        if constexpr (SupportType<ActualScaleT, half, bfloat16_t>()) {
            Reg::LoadAlign<ActualScaleT, Reg::LoadDist::DIST_UNPACK_B16>(
...
template <const QuantizeConfig& config, typename DstT, typename SrcT, typename ScaleT, typename OffsetT>
__aicore__ inline void QuantizePerGroupForKRowCommon(const LocalTensor<DstT>& dstTensor,
    const LocalTensor<SrcT>& srcTensor, const ScaleT& scale, const OffsetT& offset, const QuantizeParams& params)
{
    using ActualScaleT = typename ScaleT::PrimType;
    constexpr bool isScalarOffset = TypeUtils::IsInnerDefaultType<OffsetT>();
...
__simd_callee__ inline void QuantizePerGroupForKRowFp4OneRow(__ubuf__ DstT *dstAddr, __ubuf__ SrcT *srcAddr,
    __ubuf__ ActualScaleT *scaleAddr, Reg::RegTensor<DstT> &dstVreg,
    Reg::RegTensor<float> &srcVreg,Reg::RegTensor<float> &scaleVreg,
...
template <const QuantizeConfig& config, typename DstT, typename SrcT, typename ScaleT>
__aicore__ inline void QuantizePerGroupForKRowFp4(const LocalTensor<DstT>& dstTensor,
    const LocalTensor<SrcT>& srcTensor, const ScaleT& scale, const QuantizeParams& params)
{
    using ActualScaleT = typename ScaleT::PrimType;
    __ubuf__ DstT* dstUb = (__ubuf__ DstT*)dstTensor.GetPhyAddr();
...
#if defined(UNDEF_ASCENDC_INCLUDE_INTERNAL_HEADERS_QUANTIZATION_QUANTIZE_QUANTIZE_IMPL_H)
#undef _ASCENDC_INCLUDE_INTERNAL_HEADERS_
#undef UNDEF_ASCENDC_INCLUDE_INTERNAL_HEADERS_QUANTIZATION_QUANTIZE_QUANTIZE_IMPL_H
#endif
