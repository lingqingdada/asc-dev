...
#pragma message("impl/adv_api/detail/normalization/layernorm/layernorm_common_impl.h is an internal header file and must not be used directly. Functions or variables defined in this file may be removed in the future. Please use \"#include \"adv_api/normalization/layernorm.h\"\" and use public functions or variables defined in interface headers files.")
    ...
    const LocalTensor<T>& beta, const LocalTensor<uint8_t>& sharedTmpBuffer, const T epsilon, LayerNormTiling& tiling)
{
    ...
    CHECK_FUNC_HIGHLEVEL_API(LayerNorm, (T, isReuseSource), (output, outputMean, outputVariance, inputX, gamma, beta,
...
__aicore__ inline void LayerNormImpl(const LocalTensor<T>& output,  const LocalTensor<float>& outputMean,
    ...
    const LayerNormPara& para, const LayerNormSeparateTiling& tiling) {
        CHECK_FUNC_HIGHLEVEL_API(LayerNorm, (U, T, isReuseSource, config), (output, outputMean, outputRstd, inputX, gamma, beta, epsilon,
...
__aicore__ inline void WelfordUpdateImpl(const LocalTensor<U>& outputMean, const LocalTensor<U>& outputVariance,
    ...
    const LocalTensor<uint8_t>& sharedTmpBuffer, const WelfordUpdateParam& para)
{
    CHECK_FUNC_HIGHLEVEL_API(WelfordUpdate, (T, U, isReuseSource, config), (outputMean, outputVariance, inputMean, inputVariance, inputX,
    ...
    ASCENDC_ASSERT((para.abComputeLength > 0), {
        KERNEL_LOG(KERNEL_ERROR,
        "Failed to check para.abComputeLength, para.abComputeLength (%u) should be greater than 0.",
            para.abComputeLength, para.abLength);
    });
