    ...
    const uint32_t srcShape[], bool srcInnerPad)
{
    ...
    CHECK_FUNC_HIGHLEVEL_API(
        ReduceMax, (T, pattern), (dstTensor, srcTensor, sharedTmpBuffer, srcShape, srcInnerPad, padLast));
    ...
    if constexpr (IsSameType<pattern, Pattern::Reduce::AR>::value) {
        ...
        BinaryReduceByFirstAxis<T, isReuseSource, Max<T, false>>(dstTensor, srcTensor, tmpTensor, first, last, padLast);
    }
    SetMaskNorm();
    ResetMask();
}
