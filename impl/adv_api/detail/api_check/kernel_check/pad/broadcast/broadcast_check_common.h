...
private:
    ...
    __aicore__ inline void VerifyingParameters(const LocalTensor<T> &dstLocal, const LocalTensor<T> &srcLocal,
        const uint32_t dstShape[dim], const uint32_t srcShape[dim], LocalTensor<uint8_t> &sharedTmpBuffer) {
        ASCENDC_ASSERT(((dim == 1 || dim == 2) || HighLevelAPIParametersPrint), { KERNEL_LOG(KERNEL_ERROR,
            "[Broadcast] The dim parameter cannot be %u, should be 1 or 2.", dim); });
        ASCENDC_ASSERT(((axis == 1 || axis == 0) || HighLevelAPIParametersPrint), { KERNEL_LOG(KERNEL_ERROR,
            "[Broadcast] The axis parameter cannot be %u, should be 0 or 1.", axis); });
#if __NPU_ARCH__ == 2002
        if (dim == 2 && axis == 1) {
            ASCENDC_ASSERT((srcShape[0] * sizeof(T) % ONE_BLK_SIZE == 0 || HighLevelAPIParametersPrint), { KERNEL_LOG(KERNEL_ERROR,
                "[Broadcast] The result of srcShape[0] * sizeof(T) is %lu, "
                "should be an integer multiple of 32 when dim = 2 and axis = 1.", srcShape[0] * sizeof(T)); });
        }
#endif
        if (dim == 2 && axis == 0) {
            ASCENDC_ASSERT((srcShape[1] * sizeof(T) % ONE_BLK_SIZE == 0 || HighLevelAPIParametersPrint), { KERNEL_LOG(KERNEL_ERROR,
                "[Broadcast] The result of srcShape[1] * sizeof(T) is %lu, "
                "should be an integer multiple of 32 when dim = 2 and axis = 0.", srcShape[1] * sizeof(T)); });
        }
    }
...
class CheckFuncClassBroadcast : public DataTypeCheckFuncBasicClass,
    ...
    public MultipleTensorCheckFuncBasicClass, public CheckBroadcastParamsClass {
public:
    __aicore__ inline CheckFuncClassBroadcast() {};
    __aicore__ inline CheckFuncClassBroadcast(__gm__ const char *apiName) : DataTypeCheckFuncBasicClass(apiName),
