...
public:
    ...
    __aicore__ inline void CheckAscendDequantParams(const LocalTensor<dstT>& dstTensor, const LocalTensor<int32_t>& srcTensor,
...
private:
    ...
    __aicore__ inline void VerifyingParameters(const LocalTensor<dstT>& dstTensor, const LocalTensor<int32_t>& srcTensor,
        ...
        ASCENDC_ASSERT((params.calCount > 0 && params.calCount <= params.n) || HighLevelAPIParametersPrint, {
            KERNEL_LOG(KERNEL_ERROR,
            "[AscendDequant] params.calCount is %u, which should be in range (0, params.n %u]", params.calCount, params.n);});
        ...
        ASCENDC_ASSERT((params.m * alignInner <= dstTensor.GetSize()) || HighLevelAPIParametersPrint, {
            KERNEL_LOG(KERNEL_ERROR,
            "[AscendDequant] dstTensor element num should be not less than %u", params.m * alignInner);});
        // params.calCount must <= deqScale element num
        ASCENDC_ASSERT((params.calCount <= deqScale.GetSize() || HighLevelAPIParametersPrint), {
            KERNEL_LOG(KERNEL_ERROR, "params.calCount %u must not be \
            larger than deqScale element num %u", params.calCount, deqScale.GetSize()); });
    }

    template <typename dstT, typename scaleT, bool isPureDqParams, DeQuantMode mode>
    __aicore__ inline void PrintParameters(const LocalTensor<dstT>& dstTensor, const LocalTensor<int32_t>& srcTensor,
        const LocalTensor<scaleT>& deqScale, const LocalTensor<uint8_t>& sharedTmpBuffer, DequantParams& params,
        uint32_t calCount) {
        KERNEL_LOG(KERNEL_INFO, "[AscendDequant] The params.m is %u, params.n is %u, params.calCount is %u.",
            params.m, params.n, params.calCount);
    }
};

template <typename dstT, typename scaleT, bool isPureDqParams, DeQuantMode mode>
class CheckFuncClassAscendDequant : public DataTypeCheckFuncBasicClass, public SingleTensorCheckFuncBasicClass,
    public MultipleTensorCheckFuncBasicClass, public CheckAscendDequantParamsClass {
public:
...
public:
    __aicore__ inline void VerifyingParameters(const LocalTensor<dstT>& dstTensor, const LocalTensor<int32_t>& srcTensor,
        ...
        MultipleTensorCheckFuncBasicClass::TensorReuseVerifyingParameters(
            VA_ARGS_TO_MAKE_TUPLE(dstTensor, deqScale));
...
public:
    template <typename dstT, typename scaleT, bool isPureDqParams, DeQuantMode mode>
    __aicore__ inline void CheckAscendDequantParams(const LocalTensor<dstT>& dstTensor,
        const LocalTensor<int32_t>& srcTensor, const scaleT deqScale, const LocalTensor<uint8_t>& sharedTmpBuffer,
        DequantParams& params) {
        VerifyingParameters<dstT, scaleT, isPureDqParams, mode>(dstTensor, srcTensor, deqScale, sharedTmpBuffer, params);
        if constexpr (HighLevelAPIParametersPrint) {
            PrintParameters<dstT, scaleT, isPureDqParams, mode>(dstTensor, srcTensor, deqScale, sharedTmpBuffer, params);
        }
    }
...
private:
    ...
    __aicore__ inline void VerifyingParameters(const LocalTensor<dstT>& dstTensor,
        ...
        // params.calCount must be in range (0, params.n]
        ASCENDC_ASSERT((params.calCount > 0 && params.calCount <= params.n) || HighLevelAPIParametersPrint, {
            KERNEL_LOG(KERNEL_ERROR, "[AscendDequant] params.calCount is %u, which should be in range (0, params.n %u]",
            params.calCount, params.n);});
        // params.m * (params.n after align with dstT) <= dstTensor elementNum
        uint32_t oneBlockNum = ONE_BLK_SIZE / sizeof(dstT);
        uint32_t alignInner = (params.n + oneBlockNum - 1) / oneBlockNum * oneBlockNum;
        ASCENDC_ASSERT((params.m * alignInner <= dstTensor.GetSize()) || HighLevelAPIParametersPrint, {
            KERNEL_LOG(KERNEL_ERROR, "[AscendDequant] dstTensor element num should be not less than %u",
            params.m * alignInner);});
    }

    template <typename dstT, typename scaleT, bool isPureDqParams, DeQuantMode mode>
    __aicore__ inline void PrintParameters(const LocalTensor<dstT>& dstTensor,
        const LocalTensor<int32_t>& srcTensor, const scaleT deqScale, const LocalTensor<uint8_t>& sharedTmpBuffer,
        DequantParams& params) {
        KERNEL_LOG(KERNEL_INFO, "[AscendDequant] The params.m is %u, params.n is %u, params.calCount is %u.",
            params.m, params.n, params.calCount);
...
template <typename dstT, typename scaleT, bool isPureDqParams, DeQuantMode mode>
class CheckFuncClassAscendDequantScalar : public DataTypeCheckFuncBasicClass, public SingleTensorCheckFuncBasicClass,
    public MultipleTensorCheckFuncBasicClass, public CheckAscendDequantScalarParamsClass {
public:
