...
#pragma message("impl/adv_api/detail/api_check/kernel_check/math/math_common_check.h is an internal header file and must not be used directly. Functions or variables defined in this file may be removed in the future.")
...
class CheckFuncClassMathCommon : public DataTypeCheckFuncBasicClass, public CalCountCheckFuncBasicClass,
...
public:
    ...
    __aicore__ inline void CommonCheck(const LocalTensor<T>& dstTensor, const LocalTensor<T>& srcTensor,
    ...
    template <typename T, bool isReuseSource>
    __aicore__ inline void CommonVerifyingParameters(const LocalTensor<T>& dstTensor, const LocalTensor<T>& srcTensor,
        const LocalTensor<uint8_t>& sharedTmpBuffer, const uint32_t calCount) {
    ...
    template <typename T, bool isReuseSource>
    __aicore__ inline void CommonVerifyNoTmpBufReuse(const LocalTensor<T>& dstTensor, const LocalTensor<T>& srcTensor,
        const LocalTensor<uint8_t>& sharedTmpBuffer, const uint32_t calCount) {
        CommonCheck<T, isReuseSource>(dstTensor, srcTensor, sharedTmpBuffer, calCount);
