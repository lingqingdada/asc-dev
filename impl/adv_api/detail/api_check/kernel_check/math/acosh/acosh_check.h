...
#define IMPL_API_CHECK_KERNEL_CHECK_MATH_ACOSH_ACOSH_CHECK_H_

#if defined(__NPU_ARCH__) && (__NPU_ARCH__ == 2002 || __NPU_ARCH__ == 2201)
#include "acosh_check_common.h"
#else
#include "acosh_check_aicore.h"
#endif
...
__aicore__ inline void CheckFuncAcosh(__gm__ const char *apiName, const LocalTensor<T>& dstTensor, const LocalTensor<T>& srcTensor,
    const LocalTensor<uint8_t>& sharedTmpBuffer, const uint32_t calCount)
{
    CheckFuncClassAcosh<T, isReuseSource> checkFun(apiName);
