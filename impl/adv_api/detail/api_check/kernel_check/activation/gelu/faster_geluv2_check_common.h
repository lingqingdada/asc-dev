...
public:
    ...
    __aicore__ inline void CheckFasterGeluV2Params(const LocalTensor<T>& dstLocal, const LocalTensor<T>& srcLocal,
...
private:
    ...
    __aicore__ inline void VerifyingParameters(const LocalTensor<T>& dstLocal,
        ...
        if (std::is_same<T, float>::value) {
            ASCENDC_LOG_IF_CHECK((highPrecision == false || HighLevelAPIParametersPrint), { KERNEL_LOG(KERNEL_WARN,
                "[FasterGeluV2] The highPrecision is true, may not be effective."); });
        }
    }
    ...
    __aicore__ inline void PrintParameters(const LocalTensor<T>& dstLocal, const LocalTensor<T>& srcLocal,
        ...
        KERNEL_LOG(KERNEL_INFO,
            "[FasterGeluV2] The highPrecision is %d, highPerformance is %d.", highPrecision, highPerformance);
    }
};

template <typename T, bool highPrecision = false, bool highPerformance = false>
class CheckFuncClassFasterGeluV2 : public DataTypeCheckFuncBasicClass, public CalCountCheckFuncBasicClass,
    public SingleTensorCheckFuncBasicClass, public MultipleTensorCheckFuncBasicClass, public CheckFasterGeluV2ParamsClass {
public:
