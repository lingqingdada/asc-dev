...
template <typename T, bool isReuseSource = false>
class CheckFuncClassReGlu : public DataTypeCheckFuncBasicClass, public CalCountCheckFuncBasicClass,
    public ReuseSourceCheckFuncBasicClass, public SingleTensorCheckFuncBasicClass,
    public MultipleTensorCheckFuncBasicClass, public CheckReGluParamsClass {
public:
    __aicore__ inline CheckFuncClassReGlu() {};
    __aicore__ inline CheckFuncClassReGlu(__gm__ const char *apiName) : DataTypeCheckFuncBasicClass(apiName),
...
public:
    __aicore__ inline void VerifyingParameters(const LocalTensor<T>& dstTensor, const LocalTensor<T>& srcTensor0,
        ...
        SingleTensorCheckFuncBasicClass::TensorVerifyingParameters(VA_ARGS_TO_MAKE_TUPLE(srcTensor0, srcTensor1,
            dstTensor, sharedTmpBuffer), VA_ARGS_TO_MAKE_TUPLE_STRING(TPosition::VECIN, TPosition::VECOUT,
            TPosition::VECCALC));
        ...
        CalCountCheckFuncBasicClass::CalCountVerifyingParameters(ARG_AND_STRING(calCount),
            VA_ARGS_TO_MAKE_TUPLE(dstTensor, srcTensor0, srcTensor1));

        CheckReGluParamsClass::CheckReGluParams<T, isReuseSource>(dstTensor, srcTensor0, srcTensor1, sharedTmpBuffer, calCount);
    };
};
