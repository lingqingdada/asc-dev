...
public:
    ...
    __aicore__ inline void CheckSoftMaxFlashSameTParams(const LocalTensor<T> &dstTensor, const LocalTensor<T> &sumTensor,
        ...
        VerifyingParameters<T, isReuseSource, isBasicBlock>(dstTensor,
            sumTensor, maxTensor, srcTensor, expMaxTensor, inSumTensor, inMaxTensor, tiling, isUpdate, softmaxShapeInfo);
        if constexpr (HighLevelAPIParametersPrint) {
            PrintParameters<T, isReuseSource, isBasicBlock>(dstTensor,
                sumTensor, maxTensor, srcTensor, expMaxTensor, inSumTensor, inMaxTensor, tiling, isUpdate, softmaxShapeInfo);
        }
    }
...
private:
    ...
    __aicore__ inline void VerifyingParameters(const LocalTensor<T> &dstTensor, const LocalTensor<T> &sumTensor,
        ...
        ASCENDC_ASSERT((softmaxShapeInfo.srcK * sizeof(T) % ONE_BLK_SIZE == 0 || HighLevelAPIParametersPrint), {
            ...
            "[SoftMaxFlash] The softmaxShapeInfo.srcK is %u, should be 32B aligned.", softmaxShapeInfo.srcK); });
        if (isBasicBlock == true && (softmaxShapeInfo.srcK != 0 || softmaxShapeInfo.srcM != 0)) {
            ...
            ASCENDC_ASSERT((ans || HighLevelAPIParametersPrint), {  KERNEL_LOG(KERNEL_ERROR,
                "[SoftMaxFlash] The softmaxShapeInfo.srcK is %u, should be less than 2048 and greater than or equal to "
            ...
            ASCENDC_ASSERT((softmaxShapeInfo.srcM % CHECK_SOFTMAXFLASH_SRCM_SIZE == 0 || HighLevelAPIParametersPrint), {
                KERNEL_LOG(KERNEL_ERROR, "[SoftMaxFlash] The softmaxShapeInfo.srcM is %u, should be an integer multiple of 8.",
        ...
        ASCENDC_ASSERT((softmaxShapeInfo.srcK * softmaxShapeInfo.srcM <= srcTensor.GetSize() || HighLevelAPIParametersPrint), {
            KERNEL_LOG(KERNEL_ERROR, "[SoftMaxFlash] The softmaxShapeInfo.srcK is %u, softmaxShapeInfo.srcM is %u, "
        ...
        ASCENDC_ASSERT((srcTensor.GetSize() == dstTensor.GetSize() || HighLevelAPIParametersPrint), {
            KERNEL_LOG(KERNEL_ERROR, "[SoftMaxFlash] The dstTensor size is %u, should be equal to srcTensor size %u.",
            dstTensor.GetSize(), srcTensor.GetSize()); });
    }

    template <typename T, bool isReuseSource = false, bool isBasicBlock = false>
    __aicore__ inline void PrintParameters(const LocalTensor<T> &dstTensor, const LocalTensor<T> &sumTensor,
        const LocalTensor<T> &maxTensor, const LocalTensor<T> &srcTensor, const LocalTensor<T> &expMaxTensor,
        const LocalTensor<T> &inSumTensor, const LocalTensor<T> &inMaxTensor,
        const SoftMaxTiling &tiling, bool isUpdate, const SoftMaxShapeInfo &softmaxShapeInfo) {
        KERNEL_LOG(KERNEL_INFO, "[SoftMaxFlash] The softmaxShapeInfo.srcK is %u, softmaxShapeInfo.srcM is %u, "
...
template <typename T, bool isReuseSource = false, bool isBasicBlock = false>
class CheckFuncClassSoftmaxFlashSameT : public DataTypeCheckFuncBasicClass, public ReuseSourceCheckFuncBasicClass,
...
public:
    ...
    __aicore__ inline CheckFuncClassSoftmaxFlashSameT(__gm__ const char *apiName) : DataTypeCheckFuncBasicClass(apiName),
        SingleTensorCheckFuncBasicClass(apiName), ReuseSourceCheckFuncBasicClass(apiName),
...
public:
    __aicore__ inline void VerifyingParameters(const LocalTensor<T> &dstTensor, const LocalTensor<T> &sumTensor,
        ...
        SingleTensorCheckFuncBasicClass::TensorVerifyingParameters(
            ...
            sharedTmpBuffer), VA_ARGS_TO_MAKE_TUPLE_STRING(TPosition::VECIN, TPosition::VECOUT, TPosition::VECCALC));
        ...
        CheckSoftMaxFlashSameTParamsClass::CheckSoftMaxFlashSameTParams<T, isReuseSource, isBasicBlock>(dstTensor,
            sumTensor, maxTensor, srcTensor, expMaxTensor, inSumTensor, inMaxTensor, tiling, isUpdate, softmaxShapeInfo);
    };
...
public:
    template <typename T, bool isReuseSource = false, bool isBasicBlock = false>
    __aicore__ inline void CheckSoftMaxFlashParams(const LocalTensor<half> &dstTensor, const LocalTensor<float> &sumTensor,
        ...
        const SoftMaxTiling &tiling, bool isUpdate, const SoftMaxShapeInfo &softmaxShapeInfo) {
        VerifyingParameters<T, isReuseSource, isBasicBlock>(dstTensor, sumTensor,
            maxTensor, srcTensor, expMaxTensor, inSumTensor, inMaxTensor, tiling, isUpdate, softmaxShapeInfo);
        if constexpr (HighLevelAPIParametersPrint) {
            PrintParameters<T, isReuseSource, isBasicBlock>(dstTensor, sumTensor,
                maxTensor, srcTensor, expMaxTensor, inSumTensor, inMaxTensor, tiling, isUpdate, softmaxShapeInfo);
        }
    }
...
private:
    ...
    template <typename T, bool isReuseSource = false, bool isBasicBlock = false>
    __aicore__ inline void PrintParameters(const LocalTensor<half> &dstTensor, const LocalTensor<float> &sumTensor,
        const LocalTensor<float> &maxTensor, const LocalTensor<half> &srcTensor, const LocalTensor<half> &expMaxTensor,
        const LocalTensor<float> &inSumTensor, const LocalTensor<float> &inMaxTensor,
        const SoftMaxTiling &tiling, bool isUpdate, const SoftMaxShapeInfo &softmaxShapeInfo) {
        KERNEL_LOG(KERNEL_INFO, "[SoftMaxFlash] The softmaxShapeInfo.srcK is %u, softmaxShapeInfo.srcM is %u, "
...
template <typename T, bool isReuseSource = false, bool isBasicBlock = false>
class CheckFuncClassSoftmaxFlash : public DataTypeCheckFuncBasicClass, public SingleTensorCheckFuncBasicClass,
    public ReuseSourceCheckFuncBasicClass, public MultipleTensorCheckFuncBasicClass, public CheckSoftMaxFlashParamsClass {
public:
    __aicore__ inline CheckFuncClassSoftmaxFlash() {};
    __aicore__ inline CheckFuncClassSoftmaxFlash(__gm__ const char *apiName) : ReuseSourceCheckFuncBasicClass(apiName),
...
public:
    __aicore__ inline void VerifyingParameters(const LocalTensor<half> &dstTensor, const LocalTensor<float> &sumTensor,
        ...
        SingleTensorCheckFuncBasicClass::TensorVerifyingParameters(VA_ARGS_TO_MAKE_TUPLE(dstTensor, sumTensor,
            maxTensor, srcTensor, expMaxTensor, inSumTensor, inMaxTensor, sharedTmpBuffer),
            VA_ARGS_TO_MAKE_TUPLE_STRING(TPosition::VECIN, TPosition::VECOUT, TPosition::VECCALC));
        ...
        CheckSoftMaxFlashParamsClass::CheckSoftMaxFlashParams<T, isReuseSource, isBasicBlock>(dstTensor, sumTensor,
            maxTensor, srcTensor, expMaxTensor, inSumTensor, inMaxTensor, tiling, isUpdate, softmaxShapeInfo);
    };
