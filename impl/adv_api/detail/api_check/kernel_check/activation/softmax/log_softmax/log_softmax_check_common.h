...
public:
    ...
    __aicore__ inline void CheckLogSoftMaxParams(const LocalTensor<T>& dst, const LocalTensor<T>& sumTensor,
...
private:
    ...
    __aicore__ inline void VerifyingParameters(const LocalTensor<T>& dst, const LocalTensor<T>& sumTensor,
        ...
        ASCENDC_ASSERT((softmaxShapeInfo.srcK * sizeof(T) % ONE_BLK_SIZE == 0 || HighLevelAPIParametersPrint), {
            ...
            "[LogSoftMax] The softmaxShapeInfo.srcK is %u, should be 32B aligned.", softmaxShapeInfo.srcK); });
        ASCENDC_ASSERT((softmaxShapeInfo.srcK * softmaxShapeInfo.srcM <= src.GetSize() || HighLevelAPIParametersPrint), {
            KERNEL_LOG(KERNEL_ERROR, "[LogSoftMax] The softmaxShapeInfo.srcK is %u, softmaxShapeInfo.srcM is %u, "
            "the product of softmaxShapeInfo.srcM and softmaxShapeInfo.srcK should not be greater than src size %u.",
            softmaxShapeInfo.srcK, softmaxShapeInfo.srcM, src.GetSize()); });
    }

    template <typename T, bool isReuseSource = false, bool isDataFormatNZ = false>
    __aicore__ inline void PrintParameters(const LocalTensor<T>& dst, const LocalTensor<T>& sumTensor,
        const LocalTensor<T>& maxTensor, const LocalTensor<T>& src, const LocalTensor<uint8_t>& sharedTmpBuffer,
        const LogSoftMaxTiling& tiling, const SoftMaxShapeInfo& softmaxShapeInfo = {}) {
        KERNEL_LOG(KERNEL_INFO, "[LogSoftMax] The softmaxShapeInfo.srcK is %u, softmaxShapeInfo.srcM is %u, "
...
class CheckFuncClassLogSoftMax : public DataTypeCheckFuncBasicClass,
    ...
    public MultipleTensorCheckFuncBasicClass, public CheckLogSoftMaxParamsClass {
public:
    __aicore__ inline CheckFuncClassLogSoftMax() {};
    __aicore__ inline CheckFuncClassLogSoftMax(__gm__ const char *apiName) : DataTypeCheckFuncBasicClass(apiName),
