...
public:
    ...
    __aicore__ inline void CheckSoftMaxParams(const LocalTensor<T1>& dstTensor, const LocalTensor<T2>& sumTensor,
        const LocalTensor<T2>& maxTensor, const LocalTensor<T1>& srcTensor, const LocalTensor<float>& sharedTmpBuffer,
...
private:
    ...
    __aicore__ inline void VerifyingParameters(const LocalTensor<T1>& dstTensor, const LocalTensor<T2>& sumTensor,
        ...
        ASCENDC_ASSERT((softmaxShapeInfo.srcK * sizeof(T1) % ONE_BLK_SIZE == 0 || HighLevelAPIParametersPrint), {
            ...
            "[SoftMax] The softmaxShapeInfo.srcK is %u, should be 32B aligned.", softmaxShapeInfo.srcK); });
        ASCENDC_ASSERT((softmaxShapeInfo.srcK * softmaxShapeInfo.srcM <= srcTensor.GetSize() || HighLevelAPIParametersPrint), {
            KERNEL_LOG(KERNEL_ERROR, "[SoftMax] The softmaxShapeInfo.srcK is %u, softmaxShapeInfo.srcM is %u, "
        ...
        ASCENDC_ASSERT((dstTensor.GetSize() == srcTensor.GetSize() || HighLevelAPIParametersPrint), {
            KERNEL_LOG(KERNEL_ERROR, "[SoftMax] The dstTensor size is %u, should be equal to srcTensor size %u.",
        ...
        ASCENDC_LOG_IF_CHECK((ans || HighLevelAPIParametersPrint), {
            KERNEL_LOG(KERNEL_WARN, "[SoftMax] The config.isCheckTiling is %d, config.oriSrcM is %u, "
        ...
        if (isBasicBlock == true && (softmaxShapeInfo.srcK != 0 || softmaxShapeInfo.srcM != 0)) {
            ...
            ASCENDC_ASSERT((softmaxShapeInfo.srcM % CHECK_SOFTMAX_SRCM_SIZE == 0 || HighLevelAPIParametersPrint), {
                KERNEL_LOG(KERNEL_ERROR, "[SoftMax] The softmaxShapeInfo.srcM is %u, should be an integer multiple of 8.",
    ...
    template <typename T1, typename T2, bool isReuseSource = false, bool isBasicBlock = false, bool isDataFormatNZ = false,
        const SoftmaxConfig& config = SOFTMAX_DEFAULT_CFG>
    __aicore__ inline void PrintParameters(const LocalTensor<T1>& dstTensor, const LocalTensor<T2>& sumTensor,
        const LocalTensor<T2>& maxTensor, const LocalTensor<T1>& srcTensor, const LocalTensor<float>& sharedTmpBuffer,
        const SoftMaxTiling& tiling, const SoftMaxShapeInfo& softmaxShapeInfo) {
        KERNEL_LOG(KERNEL_INFO, "[SoftMax] The softmaxShapeInfo.srcK is %u, softmaxShapeInfo.srcM is %u, "
...
template <typename T1, typename T2, bool isReuseSource = false, bool isBasicBlock = false, bool isDataFormatNZ = false,
    const SoftmaxConfig& config = SOFTMAX_DEFAULT_CFG>
class CheckFuncClassSoftMax : public DataTypeCheckFuncBasicClass, public ReuseSourceCheckFuncBasicClass,
    public SingleTensorCheckFuncBasicClass, public MultipleTensorCheckFuncBasicClass, public CheckSoftMaxParamsClass {
public:
    __aicore__ inline CheckFuncClassSoftMax() {};
    __aicore__ inline CheckFuncClassSoftMax(__gm__ const char *apiName) : DataTypeCheckFuncBasicClass(apiName),
...
public:
    template <typename T, bool isReuseSource = false, bool isBasicBlock = false,
        const SoftmaxConfig& config = SOFTMAX_DEFAULT_CFG>
    __aicore__ inline void CheckSoftMaxSameTParams(const LocalTensor<T>& dstTensor, const LocalTensor<T>& srcTensor,
        const LocalTensor<float>& sharedTmpBuffer, const SoftMaxTiling& tiling,
        const SoftMaxShapeInfo& softmaxShapeInfo) {
        VerifyingParameters<T, isReuseSource, isBasicBlock, config>(dstTensor, srcTensor, sharedTmpBuffer, tiling, softmaxShapeInfo);
        if constexpr (HighLevelAPIParametersPrint) {
            PrintParameters<T, isReuseSource, isBasicBlock, config>(dstTensor, srcTensor, sharedTmpBuffer, tiling, softmaxShapeInfo);
        }
    }
...
template <typename T, bool isReuseSource = false, bool isBasicBlock = false,
    const SoftmaxConfig& config = SOFTMAX_DEFAULT_CFG>
class CheckFuncClassSoftMaxSameT : public DataTypeCheckFuncBasicClass, public ReuseSourceCheckFuncBasicClass,
...
public:
    ...
    __aicore__ inline CheckFuncClassSoftMaxSameT(__gm__ const char *apiName) : DataTypeCheckFuncBasicClass(apiName),
        ReuseSourceCheckFuncBasicClass(apiName), SingleTensorCheckFuncBasicClass(apiName),
        MultipleTensorCheckFuncBasicClass(apiName) {};
