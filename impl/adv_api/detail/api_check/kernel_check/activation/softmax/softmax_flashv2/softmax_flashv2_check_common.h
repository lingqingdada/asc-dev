...
public:
    ...
    __aicore__ inline CheckSoftMaxFlashV2ParamsClass(__gm__ const char *apiName) {
...
public:
    ...
    __aicore__ inline void CheckSoftMaxFlashV2Params(const LocalTensor<T1>& dstTensor, const LocalTensor<T2>& expSumTensor,
        const LocalTensor<T2>& maxTensor, const LocalTensor<T1>& srcTensor, const LocalTensor<T1>& expMaxTensor,
        const LocalTensor<T2>& inExpSumTensor, const LocalTensor<T2>& inMaxTensor, const LocalTensor<float>& sharedTmpBuffer,
...
private:
    template <typename T1, typename T2, bool isUpdate, bool isReuseSource, bool isBasicBlock, bool isDataFormatNZ,
        const SoftmaxConfig& config>
    __aicore__ inline void VerifyingParametersCommonVersion(const LocalTensor<T1>& dstTensor, const LocalTensor<T2>& expSumTensor,
        const LocalTensor<T2>& maxTensor, const LocalTensor<T1>& srcTensor, const LocalTensor<T1>& expMaxTensor,
        const LocalTensor<T2>& inExpSumTensor, const LocalTensor<T2>& inMaxTensor, const LocalTensor<float>& sharedTmpBuffer,
        const SoftMaxTiling& tiling, const SoftMaxShapeInfo& softmaxShapeInfo) 
    {
        ASCENDC_ASSERT((softmaxShapeInfo.srcK * sizeof(T1) % ONE_BLK_SIZE == 0 || HighLevelAPIParametersPrint), {
            ...
            "[%s] The softmaxShapeInfo.srcK is %u, should be 32B aligned.", apiName, softmaxShapeInfo.srcK); });
        ASCENDC_ASSERT((softmaxShapeInfo.srcK * softmaxShapeInfo.srcM <= srcTensor.GetSize() || HighLevelAPIParametersPrint), {
            KERNEL_LOG(KERNEL_ERROR, "[%s] The softmaxShapeInfo.srcK is %u, softmaxShapeInfo.srcM is %u, "
        ...
        ASCENDC_ASSERT((dstTensor.GetSize() == srcTensor.GetSize() || HighLevelAPIParametersPrint), {
            KERNEL_LOG(KERNEL_ERROR, "[%s] The dstTensor size is %u, should be equal to srcTensor size %u.", apiName,
            dstTensor.GetSize(), srcTensor.GetSize()); });
    }

    template <typename T1, typename T2, bool isUpdate, bool isReuseSource, bool isBasicBlock, bool isDataFormatNZ,
        const SoftmaxConfig& config>
    __aicore__ inline void VerifyingParametersVersion300(const LocalTensor<T1>& dstTensor, const LocalTensor<T2>& expSumTensor,
        const LocalTensor<T2>& maxTensor, const LocalTensor<T1>& srcTensor, const LocalTensor<T1>& expMaxTensor,
        const LocalTensor<T2>& inExpSumTensor, const LocalTensor<T2>& inMaxTensor, const LocalTensor<float>& sharedTmpBuffer,
        const SoftMaxTiling& tiling, const SoftMaxShapeInfo& softmaxShapeInfo) 
    {
        ...
        ASCENDC_LOG_IF_CHECK((ans || HighLevelAPIParametersPrint), {
            KERNEL_LOG(KERNEL_WARN, "[%s] The config.isCheckTiling is %d, config.oriSrcM is %u, "
    ...
    template <typename T1, typename T2, bool isUpdate, bool isReuseSource, bool isBasicBlock, bool isDataFormatNZ,
        const SoftmaxConfig& config>
    __aicore__ inline void VerifyingParametersVersion200(const LocalTensor<T1>& dstTensor, const LocalTensor<T2>& expSumTensor,
        const LocalTensor<T2>& maxTensor, const LocalTensor<T1>& srcTensor, const LocalTensor<T1>& expMaxTensor,
        const LocalTensor<T2>& inExpSumTensor, const LocalTensor<T2>& inMaxTensor, const LocalTensor<float>& sharedTmpBuffer,
        const SoftMaxTiling& tiling, const SoftMaxShapeInfo& softmaxShapeInfo) 
    {
    ...
    template <typename T1, typename T2, bool isUpdate, bool isReuseSource, bool isBasicBlock, bool isDataFormatNZ,
        const SoftmaxConfig& config>
    __aicore__ inline void VerifyingParametersVersion220(const LocalTensor<T1>& dstTensor, const LocalTensor<T2>& expSumTensor,
        const LocalTensor<T2>& maxTensor, const LocalTensor<T1>& srcTensor, const LocalTensor<T1>& expMaxTensor,
        const LocalTensor<T2>& inExpSumTensor, const LocalTensor<T2>& inMaxTensor, const LocalTensor<float>& sharedTmpBuffer,
        const SoftMaxTiling& tiling, const SoftMaxShapeInfo& softmaxShapeInfo) 
    {
        if (isBasicBlock == true && (softmaxShapeInfo.srcK != 0 || softmaxShapeInfo.srcM != 0)) {
            ...
            ASCENDC_ASSERT((ans || HighLevelAPIParametersPrint), {  KERNEL_LOG(KERNEL_ERROR,
                ...
                "256/sizeof(T), and should be an integer multiple of 64.", apiName, softmaxShapeInfo.srcK); });
            ASCENDC_ASSERT((softmaxShapeInfo.srcM % CHECK_SOFTMAXFLASHV2_SRCM_SIZE == 0 || HighLevelAPIParametersPrint), {
                KERNEL_LOG(KERNEL_ERROR, "[%s] The softmaxShapeInfo.srcM is %u, should be an integer multiple of 8.",
    ...
    {
        VerifyingParametersCommonVersion<T1, T2, isUpdate, isReuseSource, isBasicBlock, isDataFormatNZ, config>(args...);
        if constexpr (__NPU_ARCH__ == 3002) {
            VerifyingParametersVersion300<T1, T2, isUpdate, isReuseSource, isBasicBlock, isDataFormatNZ, config>(args...);
        } else if constexpr (__NPU_ARCH__ == 2002) {
            VerifyingParametersVersion200<T1, T2, isUpdate, isReuseSource, isBasicBlock, isDataFormatNZ, config>(args...);
            VerifyingParametersVersion220<T1, T2, isUpdate, isReuseSource, isBasicBlock, isDataFormatNZ, config>(args...);
    ...
    __aicore__ inline void PrintParameters(const LocalTensor<T1>& dstTensor, const LocalTensor<T2>& expSumTensor,
        ...
        const LocalTensor<T2>& inExpSumTensor, const LocalTensor<T2>& inMaxTensor, const LocalTensor<float>& sharedTmpBuffer,
        const SoftMaxTiling& tiling, const SoftMaxShapeInfo& softmaxShapeInfo) {
        KERNEL_LOG(KERNEL_INFO, "[%s] The softmaxShapeInfo.srcK is %u, softmaxShapeInfo.srcM is %u, "
            "softmaxShapeInfo.oriSrcM is %u, softmaxShapeInfo.oriSrcK is %u.", apiName, softmaxShapeInfo.srcK,
...
template <typename T1, typename T2, bool isUpdate, bool isReuseSource, bool isBasicBlock, bool isDataFormatNZ,
    const SoftmaxConfig& config>
class CheckFuncClassSoftmaxFlashV2 : public DataTypeCheckFuncBasicClass, public CheckSoftMaxFlashV2ParamsClass,
...
public:
    ...
    __aicore__ inline CheckFuncClassSoftmaxFlashV2(__gm__ const char *apiName) : DataTypeCheckFuncBasicClass(apiName),
...
public:
    ...
    __aicore__ inline void VerifyingParameters(const LocalTensor<T1>& dstTensor, const LocalTensor<T2>& outReduceMax,
        ...
        const LocalTensor<float>& sharedTmpBuffer, const SoftMaxTiling& tiling, const SoftMaxShapeInfo& softmaxShapeInfo) {
            VerifyingParameters(dstTensor, outSum, outMax, srcTensor, outexpMax, inSum, inMax,
                sharedTmpBuffer, tiling, softmaxShapeInfo);
